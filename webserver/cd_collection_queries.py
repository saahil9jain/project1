...
UPDATE_REVIEW = (
        "UPDATE reviews " 
        "SET score=(:score), review_date=(:review_date) "
        "WHERE person_id=(:person_id) AND album_id=(:album_id)"
        )
...
FIND_LARGEST_COMPANY =  (
        ...
        "   SELECT E.company_id, count (*) AS counts "
        "   FROM employs1 E "
        "   GROUP BY E.company_id ) "
        "SELECT rc.company_name "
        "FROM employeecounts, recordcompany AS rc "
        "WHERE rc.company_id = employeecounts.company_id AND "
        "   employeecounts.counts = (SELECT DISTINCT MAX (employeecounts.counts) FROM employeecounts) "
        )
FIND_HOTTEST_ALBUMS = (
